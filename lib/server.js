...
apiServer.listen(WEB_SERVICE_PORT, () => console.log(`WebService listening on port ${WEB_SERVICE_PORT}`))

// setup broker
const brokerServer = net.createServer(broker.handler)
brokerServer.listen(BROKER_PORT, () => console.log(`MQTT broker listening on port ${BROKER_PORT}`))
...
websocketBrokerServer.listen(BROKER_WEB_SOCKET_PORT, () => console.log(`MQTT broker server listening on port ${BROKER_WEB_SOCKET_PORT}`))
